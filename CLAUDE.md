...
- A series key is `<namespace>.<subject>.<measure>.<grain>`; build it with `ingest.base.series_key()` and read it back through the `observations` view's `source_ns`, `subject`, `measure` and `grain` columns. Never take a key apart with string surgery in SQL.
