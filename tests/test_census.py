...
def test_a_changed_file_or_a_table_off_the_headline_is_an_error(tmp_path, monkeypatch):
    ...
    passes = census.rows(d / "functions.csv")[0]["passes_usd"]
    (d / "functions.csv").write_text(text.replace(passes, "1" + passes, 1))
    ...
    assert any("functions.csv passes_usd sums to" in e for e in census.problems(spec, census.fetches()))


def test_a_draft_or_a_bundle_whose_verdicts_do_not_reproduce_is_an_error(tmp_path, monkeypatch):
    spec, d = _copy(tmp_path, monkeypatch)
    m = json.loads((d / "manifest.json").read_text())
    m["draft"], m["reconciliation"]["verdicts_reproduced_from_published_columns"] = True, 0.99
    (d / "manifest.json").write_text(json.dumps(m))
    errors = census.problems(spec, census.fetches())
    assert any("draft" in e for e in errors) and any("reproduce" in e for e in errors)
...
def test_every_passes_figure_travels_with_the_three_way_figure_and_nothing_modelled_leaks():
    index, roles = census.build(census.load(), census.fetches())
    for doc in [index, *roles.values()]:
        for path, d in _walk(doc):
            assert "freed" not in d, path  # v3 renamed it; an old key would mean an unmapped field
            if "passes" in d and not isinstance(d["passes"], bool) and "rule" not in d and not path.startswith("/method"):
                assert "agreed3" in d, path
            modelled = [k for k in d if "modelled" in k]
            assert modelled == [] or (path == "/headline" and modelled == ["modelled_saving"]), path
    assert "sigma" in index["method"] and set(index["headline"]["by_scorer"]) == set(index["scorers"])
...
def test_the_headline_citation_is_flat_and_leaves_out_the_modelled_saving():
    h = census.build_headline(census.load())
    assert all(isinstance(v, float) for v in h.values()) and "by_scorer_passes_usd.sonnet" in h
    assert not any("modelled" in k for k in h)
...
def test_tables_keep_codes_as_text_and_answer_after_file_access_is_locked():
    ...
    assert con.execute("SELECT count(*) FROM census_tasks").fetchone()[0] == 11176
...
def test_a_census_figure_is_citable_and_a_modelled_one_is_not():
    ...
    occ, freed, modelled = t.pub.execute(
        "SELECT occ, agreed3_usd, modelled_saving_usd FROM census_roles WHERE occ = '13-2011'"
    ).fetchone()
...
def test_the_census_words_never_claim_capability():
    """v3 is a structural screen: no "can go", no "today", and a role's strict and loose shares are not a range."""
    from pathlib import Path

    web = Path(__file__).resolve().parents[1] / "web" / "src" / "app"
    pages = [(web / "census" / "page.tsx").read_text(), (web / "census" / "roles" / "[occ]" / "page.tsx").read_text()]
    meth = (web / "methodology" / "page.tsx").read_text()
    pages.append(meth[meth.index('id="census"') :][:4000])
    for s in [*census.strings(census.load()), *pages]:
        assert not re.search(r"\bcan go\b|\btoday\b|\ba range\b", s, re.I), s[:200]
