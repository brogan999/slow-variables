    ...
    <div className="h-72 w-full" role="img" aria-label={`${unit} over time with the direction rule's dead band`}>
      <ResponsiveContainer>
        <LineChart data={pts} margin={{ top: 8, right: 16, bottom: 4, left: 0 }}>
          ...
          <Tooltip cursor={{ stroke: "var(--axis)" }} content={({ payload }) => { const d = payload?.[0]?.payload as (typeof pts)[number] | undefined; return d ? <div className="rounded bg-surface ring-hair px-2 py-1 text-xs"><div>{fmtUnit(d.v, unit)}</div><div className="text-muted">{d.as_of}</div></div> : null; }} />
