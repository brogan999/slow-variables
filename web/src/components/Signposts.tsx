...
export function JaggedFrontier({ doc }: { doc: Signposts }) {
  ...
  return (
    <Figure
      ...
      foot={<>
        <p>Each score is on its own test&apos;s scale. Most are the share of tasks solved; GDPval&apos;s is how often graders prefer the model&apos;s work to a professional&apos;s; FrontierSWE&apos;s is partial credit on each task, averaged over five runs; PostTrainBench&apos;s is the average score of the models the agent post-trained, across several benchmarks. The tests measure different things, so read each row against its own scale, not against the others. A record close to a perfect score, as on the tests at the top, leaves that test little room to show further progress. Scores sit at model release dates, often from runs with no limit on compute.</p>
        <p>A high share of tasks solved is not a measure of progress in research. <a href="/outlook#source-jones_b" className="underline decoration-grid underline-offset-2 hover:decoration-ink">Benjamin Jones</a>, an economist who studies where scientific progress comes from, argues that research needs every one of its steps done, so the steps a model still fails set the pace, however many others it solves.</p>
