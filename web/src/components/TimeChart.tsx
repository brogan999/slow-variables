...
const tip = (p: Point, unit: string) =>
  [who(p), fmt(p.value, unit) + range(p, unit), p.as_of, p.stamp, p.partial ? "quarter still running" : null, p.disputed ? "disputed" : null].filter(Boolean).join(" · ");

const Dot = ({ hollow = false, faint = false, dashed = false }) => (
  <svg width="10" height="10" aria-hidden><circle cx="5" cy="5" r="3.5" fill={hollow || dashed ? "var(--surface)" : "var(--s1)"} stroke="var(--s1)" strokeWidth="1.5" strokeDasharray={dashed ? "2 1.5" : undefined} opacity={faint ? 0.35 : 1} /></svg>
);
...
export function TimeChart({ d }: { d: Doc }) {
  ...
  return (
    <Figure
      title={c.dead ? `Direction over the last ${c.needs} readings` : "Every reading"}
      ...
      keys={<>
        ...
        {c.partial ? <Key swatch={<Dot dashed />}>a quarter still running, dated at its end</Key> : null}
        ...
        {c.dead && c.change ? <Key swatch={<Swatch className="bg-ink/5 border border-dashed border-axis" />}>moves within {c.change.dead_band} read as stable, and a move counts only if more than half the steps go its way; this window moved {c.change.label} and {c.change.steps}; higher reads {words(rule?.higher_is)}</Key> : null}
          ...
          <tbody>
            {pts.slice().reverse().map((p, i) => (
              <tr key={i}>
                ...
                <td>{p.href ? <a href={p.href} className="underline decoration-grid underline-offset-2 hover:decoration-ink">{p.stamp ?? "record"}</a> : "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
