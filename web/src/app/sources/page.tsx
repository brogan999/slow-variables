    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="overflow-x-auto">
        <table className="data w-full text-sm">
          ...
          <tbody>
            {rows.map((s) => (
              <tr key={s.id} id={s.id} className="target:bg-surface-2">
      ...
      <ul className="flex flex-col gap-1.5 text-sm">
        {[...rows.filter((s) => s.people?.length).map((s) => ({ id: s.id, name: s.name, people: s.people ?? [] })), ...skipped.filter((s) => s.people?.length).map((s) => ({ id: s.id, name: s.name, people: s.people ?? [] }))].map((g) => (
          <li key={g.id}><span className="font-medium">{g.people.join(", ")}</span> <span className="text-muted">·</span> <a href={`#${g.id}`} className="text-ink-2 underline decoration-grid underline-offset-4">{g.name}</a></li>
        ))}
      </ul>
    </div>
  );
