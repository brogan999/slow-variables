...
const industryHead = ["Industry (code)", "Payroll, all workers", "Can go", "All three agree", "Share of payroll", "Would go if software could check it"];
const industryRow = (i: CensusIndex["industries"][number]) => (
  <tr key={i.naics} id={`naics-${i.naics}`}><th scope="row">{i.title} <span className="text-muted font-mono text-[11px]">{i.naics}</span></th><td className="tabular-nums">{usd(i.payroll)}</td><td className="tabular-nums">{usd(i.freed)}</td><td className="tabular-nums"><A3 v={i.agreed3} /></td><td className="tabular-nums">{fmt(i.share_total, "share")}</td><td className="tabular-nums">{usd(i.verifier_queue)}</td></tr>
);

const OUTCOME = (p: boolean | "x" | null) => (p === true ? "passed" : p === false ? "failed" : p === "x" ? "exploratory" : "not run");

function Method({ c }: { c: CensusIndex }) {
  const m = c.method;
  const n = c.prose.method_notes;
  const who = (k: string) => (k === "vote" ? "the vote" : c.scorer_names[k] ?? k);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div>
        ...
        <Rows head={["Model", "Tasks scored", "Matches workers on stakes", "Matches workers on checking", "AI alone where it says the work can go"]} rows={m.adjudication} row={(a) => (
      ...
      <div>
        <h3 className="font-medium mb-2">How hard a task is to describe: exploratory</h3>
        <p className={note}>{n.channel}</p>
        <details className="panel px-4 py-3"><summary className="cursor-pointer">The numbers</summary>
          <Rows head={["Month", "Model", "Tasks", "Effect of harder description", "Strength"]} rows={m.channel.runs} row={(r) => <tr key={`${r.period}${r.scorer}`}><td className="tabular-nums">{r.period}</td><th scope="row">{who(r.scorer)}</th><td className="tabular-nums">{fmt(r.n, "count")}</td><td className="tabular-nums">{fmt(r.coef)}</td><td className="tabular-nums">{fmt(r.z)}</td></tr>} />
        </details>
      </div>
      <div>
        <h3 className="font-medium mb-2">The physical-work filter</h3>
        <p className="text-sm text-ink-2 max-w-[66ch]">Work that needs a body was removed before scoring: {usd(pg.removed_usd)} of payroll. A spot check anyone can rerun, of {fmt(pg.spotcheck.sample, "count")} removed tasks, found {fmt(pg.spotcheck.error_rate, "share")} were really desk work (a ninety percent interval of {band(pg.spotcheck.ci90[0], pg.spotcheck.ci90[1])}), so the total that can go may be up to {usd(pg.spotcheck.headline_low_by_usd[0])} to {usd(pg.spotcheck.headline_low_by_usd[2])} too low.</p>
      </div>
      <div>
      ...
      margin={
        <>
          <MarginPanel title={`Readings · version ${c.version}`} rows={[
            ...
            ["Would go if software could check it", usd(h.verifier_queue)],
            ["Of the payroll that can go, decided by the two Claude models alone (Gemini never scored it)", usd(h.without_gemini)],
          ]} />
          <MarginPanel title="Instrument">
            <p>Judged by {c.scorers.join(", ")}; the verdict on each task is their vote. Imported as published from the census&apos;s own files and checked against their published hashes.</p>
            <p>Download: {Object.entries(c.csv).map(([f, href], i) => <span key={f}>{i ? ", " : ""}<a href={href} className={link}>{f}</a></span>)}.</p>
            <p>Query every table in the <Link href="/query" className={link}>SQL console</Link>, e.g. <code className="text-[12px]">SELECT title, band_lo, band_hi, freed, freed_agreed3 FROM census_roles WHERE function = &apos;Finance&apos;</code>.</p>
    ...
    >
      <div className="flex flex-col gap-16">
        ...
        <Section id="dial" n="The rule" title={s.dial.title} lede={s.dial.lede}>
          <Rows head={["Rule", "Can go", "All three agree"]} rows={c.dial} row={(d) => (
            <tr key={d.rule} className={d.headline ? "bg-surface-2" : undefined}><th scope="row">{d.rule}{d.headline ? <strong className="font-medium text-ink"> (the rule used)</strong> : null}</th><td className="tabular-nums">{usd(d.freed)}</td><td className="tabular-nums">{usd(d.agreed3)}</td></tr>
          )} />
        </Section>
        ...
        <Section id="functions" n="Part II · functions" title={s.functions.title} lede={s.functions.lede}>
          <Rows head={["Function", "Knowledge payroll", "Can go", "All three agree", "Share that can go", "Would go if software could check it"]} rows={c.functions} row={(f) => (
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {c.deals.cards.map((d) => (
              <article key={d.key} id={`deal-${d.key}`} className="panel px-4 py-4 flex flex-col gap-2 text-sm">
                ...
                <p className="tabular-nums">Payroll {usd(d.payroll)} · can go {usd(d.freed)} <span className="text-muted">(from {usd(d.freed_lo)} to {usd(d.freed_hi)} as the rule tightens or loosens)</span> · all three agree <A3 v={d.agreed3} /></p>
