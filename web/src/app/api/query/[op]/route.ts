...
export const maxDuration = 120; // a careful answer with a revise turn and a retry can take well over a minute
...
export async function POST(req: Request, { params }: { params: Promise<{ op: string }> }) {
  ...
  try {
    const r = await fetch(`${url}/${op}`, {
      ...
      body: JSON.stringify(body), signal: AbortSignal.timeout(op === "ask" ? 110_000 : 30_000), // the Fly machine cold-starts in ~20 s
    });
