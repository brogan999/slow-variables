...
import { Credit, ForecastRow, IdeaCard, Plate, Sources } from "@/components/FuturesParts";
    ...
    <ArticleLayout
      head={
        <PageHeader eyebrow={<><Link href="/futures" className="hover:text-ink">Futures</Link> · kind of technology</>} title={c.name}
          lede="Dated forecasts about this kind of technology from the singularity reading list, then every idea of this kind in the order stories first imagined it.">
      ...
      margin={
        <MarginPanel title="Profit, as AI models judge it" rows={[["Ideas of this kind", fmt(c.n, "count")], ...words.filter((w) => c.tiers[w]).map((w) => [w, fmt(c.tiers[w], "count")] as [string, string])]} />
      }
    >
