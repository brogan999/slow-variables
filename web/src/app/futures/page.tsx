...
import { Credit, Plate, link } from "@/components/FuturesParts";
    ...
    <ArticleLayout
      head={
        <PageHeader
          ...
          lede="Science fiction has imagined most technologies long before anyone built them. This section lays out thousands of those ideas by the decade a story first imagined them, and by the decade they may arrive, with a judgement of who would keep the profit if they were sold."
        />
      }
      margin={
        <>
          <MarginPanel title="The collection" rows={[
            ...
            ["Dated forecasts from the canon", fmt(f.n_forecasts, "count")],
            ["Illustrated", fmt(f.n_images, "count")],
          ]} />
          <MarginPanel title="Where it comes from">
            <p>The ideas come from {f.credits.map((c, i) => <span key={c.url}>{i ? " as compiled in " : ""}<a href={c.url} className={link}>{c.name}</a></span>)}. Names, works, authors and years are theirs; every description is this site&apos;s own rewording.</p>
            <p>Categories, arrival decades and rent judgements were made by AI models, not people. <Link href="/methodology#futures" className={link}>How they were judged</Link></p>
          </MarginPanel>
        </>
      }
    >
      <div className="flex flex-col gap-16">
        <Section id="imagined" eyebrow="Part I" title="When each idea was imagined" lede="Each bar counts the ideas first imagined in that decade; the darker part is how many have since been built. It measures what the glossary records, which leans on the pulp magazine and paperback boom, not how imaginative each decade was.">
        ...
        <Section id="expected" eyebrow="Part II" title="When they may arrive" lede="For the ideas not yet built, the upper bar is the decade the AI models judged a working, commercial version most likely. The lower bar counts dated forecasts named in the books and essays of the singularity canon, set at the decade their author gave. The two are different kinds of claim and are never added together.">
          <ol className="flex flex-col gap-2">
            {f.expected.map((d) => (
              <li key={d.key} className="grid grid-cols-[9.5rem_1fr_7rem] items-center gap-3 text-[13px]">
                <Link href={d.href} className={link}>{d.label}</Link>
                <span className="flex flex-col gap-0.5" aria-hidden>
                  <span className="h-2.5 bg-surface-2 rounded-sm relative"><span className="absolute inset-y-0 left-0 bg-s1 rounded-sm" style={{ width: `${d.judged_width}%` }} /></span>
        ...
        <Section id="categories" eyebrow="Part III" title="Kinds of technology, and who would profit" lede={<>Each idea was sorted into one kind of technology and, where it could be sold, judged for the kind of profit it could earn and who would keep it. Most ideas, once built, would be copied and their profit competed away. <Link href="/methodology#futures" className={link}>How the rent is judged</Link></>}>
          <div className="grid gap-5 sm:grid-cols-2">
            {f.categories.map((c) => (
              <Link key={c.id} href={c.href} className="panel px-3 py-3 flex flex-col gap-2 hover:border-ink">
                {c.image ? <Plate src={c.image} alt={`Illustration for ${c.name}`} sizes="(min-width: 640px) 400px, 100vw" /> : null}
                <span className="font-medium">{c.name}</span>
                <span className="text-[12px] text-ink-2 tabular-nums">{fmt(c.n, "count")} ideas · {fmt(c.built, "count")} built</span>
                <span className="text-[11px] text-muted">Rent where sold: {f.tier_words.filter((w) => c.tiers[w]).map((w) => `${w} ${fmt(c.tiers[w], "count")}`).join(" · ")}</span>
              </Link>
            ))}
          </div>
          <Credit />
        ...
        <Section id="gallery" eyebrow="Part IV" title="A gallery of the shortlist" lede="The ideas picked out for a closer look, in the order they were first imagined. Each picture shows the idea as if it already existed.">
