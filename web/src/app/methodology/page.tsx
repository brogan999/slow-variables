...
export default function Methodology() {
  ...
  return (
    ...
    <section aria-labelledby="read" className="prose-folio drop-cap">
      ...
      <p>Some readings are <strong>held rather than scored</strong>. A reading within a twentieth of the line between two ranges, or one whose margin of error covers both, reads <em>emerging</em> until it moves clear. Several readings of one underlying survey count as one vote, however many publishers repeat it. A company describing its own results can never move a status past <em>emerging</em> on its own.</p>
      <p>Every source carries a <strong>grade</strong> from A to D. A is independent measurement, an audited filing or an official statistic. D is a company talking about itself, or a single source nobody has confirmed. The grade is shown beside the reading and decides how far that reading can move a conclusion.</p>
      <p><strong>Unmeasured</strong> and <strong>unpublished</strong> mean different things. Unmeasured means nothing on the site measures that stage yet. Unpublished means the idea is recorded but no trustworthy free series exists, and the reason sits on the indicator&apos;s row.</p>
