...
export default async function BucketPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {context().figures.some((f) => f.page === `buckets/${id}`) ? (
        <section className="flex flex-col gap-6">
          <div>
            ...
            <p className="text-sm text-ink-2 max-w-[62ch]">Official statistics, and figures companies publish from their own records, read beside the claims about what happens next. They carry no status of their own, and none measures AI alone; where one is also the reading behind an indicator on this page, its caption says so.</p>
          </div>
      ...
      <section>
        <h2 className="display text-2xl leading-tight mb-3 mt-2">Crosswalk to the capture lens</h2>
