...
| 2 | Every indicator published with status + counterevidence, or unpublished with a rendered reason | Done for 87 indicators (64 published); the Yale workbook connector and the OpenRouter rankings scrape landed; the Artificial Analysis connector is built and optional until the key exists. Open: neocloud debt terms, depreciation useful-life changes, enterprise multi-homing (Menlo), the acquisition ledger (rows being sourced). |
