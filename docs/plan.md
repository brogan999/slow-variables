...
| 4 | Twelve golden questions pass; citecheck tested; one memo PR produced and merged | Key live on Fly, GitHub and locally (10 Sep). Golden through the live service: 11 of 13 (g05 quotes a threshold that lives in no record; g07 cited the wrong derived row and the check blocked it, as designed). citecheck has 14 tests. The memo job produced a prose memo (claude-fable-5-1) for an empty window, closed unmerged; Monday's run is the first real one. Ask drawer answers on production. |
