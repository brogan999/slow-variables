...
| Part 6 decisions (11 Sep 2026) | No launch; the memo reaches readers by RSS only; stay free (paid-only items stay unpublished with reasons); ROIC−WACC and markups not computed (said on /methodology); the 11-layer value-chain map stays private; the five-tier dependency graph was never found; Exa not used (direct fetches and connectors instead); X posts are quoted from pages that embed them (X's robots.txt disallows its embed endpoint); the Part 1 brief and six chat exports are private and were removed from history with a fresh repo (the earlier repo, whose PR numbers #1–#10 appear below, is a private archive) | Alex's answers, 11 Sep 2026 |
| Self-merging nightly (Part 5, 11 Sep 2026) | The nightly merges its own PR when `check`, `pytest` and the web build pass on the tree it built; agent PRs may run `approve`. Departs from `v2`'s "humans merge" and "do not approve your own extraction": values are bound to their snippets by test (`coding` notes for derived values), hand-coded values and reasons are listed as judgement calls in each PR, and nothing `llm_extract` ships. A band crossing still waits for a human reason | The owner's budget is 30 minutes a week; a human merge every morning is not sustainable, and a mechanical gate is stricter than a habitual click |
...
| `sec_xbrl` | companyfacts per entity with `cik` (NVDA AMD MSFT GOOGL AMZN META ORCL CRWV NBIS; CIKs from `sec.gov/files/company_tickers.json`, fetched once). `SEC_USER_AGENT` env | `Revenues`∥`RevenueFromContractWithCustomerExcludingAssessedTax`, `GrossProfit`, `CostOfRevenue`, `PaymentsToAcquirePropertyPlantAndEquipment`→`capex`, `RevenueRemainingPerformanceObligation`→`rpo`, `DepreciationDepletionAndAmortization`→`da`. Forms 10-K/10-Q and their amendments, USD; SEC's `frame` picks one fact per calendar quarter, year or instant (a frame on an 8-K recast is read from the 10-K/10-Q with the same dates and value); year-to-date capex kept as `.h1`/`.9m`; the tag with the latest period leads, and for revenue the other tag fills the periods it lacks when the two agree on every period both report; `as_of=end`, `published=filed`. Q4 = FY − 9M as a **Derived** (M2, with margin stacking) | 4 / 10-K audited, 10-Q company_stated | `sec_nvda_companyfacts.json` (5 concepts) |
...
9. Routes: `/` diffusion L0 (`StockFlowDiagram`: hand SVG, **vertical layout for 390px**, valves coloured by status, leak → `/capture`, last three StatusEvents, "what would change our mind"), `/capture` L0 (layer list with live cards; margin-bulge vertical is M2), `/buckets/[id]`, `/layers/[id]`, `/data/[series]` (L3 table + CSV), `/crosswalk`, `/sources` (health from FetchLog + staleness `now − last as_of > 2×cadence` → "stale as of"), `/methodology` (tiers, grade derivation, vocabularies, rubric, discipline rules, Epoch + Joy Larkin credits), `/changelog` (from `status_events.jsonl`).
...
| 9 | Maintainer conflicts | None listed (11 Sep 2026) |
...
6. Name locked ≥80/100 on the canon checklist, domain live on Vercel, wordmark renamed; repo scrubbed (private business notes and personal paths out) and public; deployment protection set to previews only.
...
| 4 | Twelve golden questions pass; citecheck tested; one memo PR produced and merged | Golden, run live on 11 Sep after the query improvements (prompt v3, hints naming the record that holds a rejected number, indicator records carrying their own reading and status reason, and a blocked answer's fresh attempt escalated to Opus): three consecutive runs each passed 13 of the 14, with a different single question missing each time, and every one of those passed when re-asked on its own. Before the improvements the same set scored 11 of 14. The remaining variance is the model's, not the checker's: a number attached to the wrong record, or one quoted without a citation. citecheck is tested. The refusal case now allows figures that pass the check and requires the answer to say the store holds no such record. The memo PR carries operator notes and falls back to the digest on any model error; Monday 14 Sep is the first real run, and merging it closes this row. |
...
## Part 5 status (11 Sep 2026): finished, and it runs itself

Scope set by Alex: finish the brief, make the operating cadence automatic, add privacy and disclaimers; owner time under 30 minutes a week. Shipped in PRs #4, #5, #6, #8, #9 and the close-out PR, each with a "Judgement calls" list at the top of its body for Alex to review.

- **Cadence.** The nightly merges itself behind check, pytest and the web build, redeploys the query service through a reusable workflow whose smoke test compares /health's observation count with the build's, opens one issue per layout change, and no longer emails on a source outage. The cache key bug (the cache froze on 10 Sep) and the red zero-job CI runs on bot PRs are fixed. `check` separates errors (block the merge) from attention items (stale cards, reasons waiting more than 14 days); a waiting crossing shows on its card as "evaluator reads X; reason pending". The Monday memo PR carries operator notes and requests Alex's review; the query service suspends instead of stopping, so the daily spend ledger survives idling.
...
## Parts 7 and 8 status (12 Sep 2026): audits, instrument logic, and the key

Three read-only audits (brief against shipped, codebase structure, instrument logic) and a staff review of the Part 6 diff drove PRs #18 to #26. The product did not gain scope; it gained rules that can be stated and checked.

- **Security and provenance.** The audit log keeps only citations that resolve to a record, so nothing a visitor writes can reach the public log. Model and console SQL run on a separate locked database holding the seven public tables, which no spelling of a raw table name can reach. Each request thread gets its own cursor, because concurrent questions were reading wrong row counts.
- **One rule per thing.** The evidence grade was computed four ways and is now one, basis-aware, which moved 1,668 quarterly filings from A to B and matched the methodology page. The number formatter was four implementations and is now one, shared by the site, the evaluator and the memo. The staleness rule was written four times, once a month early, and is now one. A connector's expected series is declared once, on the connector.
- **Verdicts say what they rest on.** One instrument casts one vote, so four readings of one survey no longer outvote another instrument; `unclear` is an evaluator failure and stops voting; a reading within 5% of a band edge, or whose confidence interval spans two bands, reads `emerging`; and every bucket, valve and layer carries a tally. Three headline clauses changed: methods reads mixed, the return arrow emerging, the model layer mixed.
- **The thesis monitor can say contradicted.** Rents migrate up now reads contradicted rather than "does not hold", because chips took 60.6 to 69.5 percent of stack gross profit over four quarters. The surplus rule reads untestable behind a doubled ceiling covering the willingness-to-accept wedge. The falsification rule is guarded against the suite's ceiling censoring its own input, and circular financing reads the trailing-year flow rather than a cumulative total that could only rise.
- **The series key is named.** The observations view exposes namespace, subject, measure and grain; `ingest.base.series_key()` builds one; three formulas join on those columns with values identical on every row. The remaining formulas and connector build sites convert as each is next touched, because a mistyped key forks a series.
- **The site reads as a summary.** Lens pages show the strongest readings per stock or layer and link through, which took capture from 24,800 to 13,100 pixels on a phone and the changelog from 30,300 to 6,100. Cards name the number the status was read from when it differs from the latest point. A layer with indicators but no direction rule says so rather than reading unmeasured.
- **Charts checked against the visualisation method.** The two series marks separate by 28 to 31 in perceptual distance for normal, protan, deutan and tritan vision in both themes, and the two status accents by 24 to 28. They read as neutrals by design, since the hues are reserved for meaning. The one real fix was a hit target larger than an 8-pixel dot. /methodology states those rules.

Owner decisions, 12 September: escalate a blocked answer to Opus rather than move every answer; count one vote per instrument and print the tally; let the interval decide when it straddles a band edge; build routed-token volume; drop a seed prior that disagrees with the evaluator; unpublish an indicator that is a rescale of another; convert the remaining key sites as they are touched; fix only the drift-prone duplication and leave the file split alone.
...
- **Positions of this site's own.** Where no named writer takes the other side, the rival is labelled as this site's; seven positions carry an author's argument further than the author did, and say so.
...
- **The map's marks.** A claim with a stage says its row will bind there; a claim about what happens on a row names the row without a stage and is listed, not marked. Ten of the thirty claims mark a cell, and the reorganising column carries none of them: the claims about it are about who gains and who loses, not about what holds AI up.
...
- **Sources left out.** No private chat, private memo or paid text; The Diff only from posts that read in full logged out; Cai only on training data; a claim not found in a free text is not attributed to its supposed author.
